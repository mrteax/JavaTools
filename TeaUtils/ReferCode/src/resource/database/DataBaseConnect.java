...
public class DataBaseConnect {
	...
	private String configXmlPath="conf/config.xml";
...
//	    	stmtUpdate.close();	
	...
	/**
	   ...
	   * 注意:关闭数据连接
	   * */
	public boolean executeStatusUpdate(int site_id)
	{
	   String sql="update newssite set status=4 where site_id="+site_id;
	   try
	   {
	...
	/**
	   ...
	   * 注意:关闭数据连接
	   * */
	public String executeErrorQuery(String strUrl)
	{
	   ResultSet rs=null;
	   String result=null;
	   ...
	   String sql="select site_url from newssite,newsurl where newssite.site_id=newsurl.site_id and url='"+strUrl+"'";
	   try
	   {
	     rs=stmtQuery.executeQuery(sql);
	     rs.next();
	     result=rs.getString("site_url");
	   }
	   catch(Exception ex)
	   {
	    this.closeConnection();
	...
	public static void main(String[] args) {
		...
		DataBaseConnect dbc=new DataBaseConnect("f:/search/conf/config.xml");
